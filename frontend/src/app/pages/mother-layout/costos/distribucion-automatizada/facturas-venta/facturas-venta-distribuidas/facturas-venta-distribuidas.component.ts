...
interface FacturaVentaDistribuida {
  id: number;
  folio: string;
  tipo_doc: string;
  tipo_compra: string;
...
interface FacturaAgrupada {
  ...
  distribuciones: FacturaVentaDistribuida[];
  expanded: boolean;
  selected: boolean;
...
@Component({
  selector: 'app-facturas-venta-distribuidas',
  standalone: true,
  imports: [
      CommonModule,
      ReactiveFormsModule,
  ...
  templateUrl: './facturas-venta-distribuidas.component.html',
  styleUrl: './facturas-venta-distribuidas.component.css'
})
export class FacturasVentaDistribuidasComponent implements OnInit {
  ...
  // Datos de facturas
  facturasOriginales: FacturaVentaDistribuida[] = [];
  ...
  cargarDatosDistribucion(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.apiService.post('facturas_sii_venta_distribuidas/', {
  ...
  agruparFacturas(): void {
    const facturasPorFolio = new Map<string, FacturaVentaDistribuida[]>();
  ...
  try {
    ...
    // Formato dd/mm/yyyy (como viene de tu BD: "24/06/2025")
    if (fecha.includes('/') && fecha.split('/').length === 3) {
      ...
      fechaParseada = new Date(año, mes, dia);
    }
    // Formato ISO (2025-07-10) o otros formatos que JavaScript puede parsear
    else {
    ...
    if (isNaN(fechaParseada.getTime())) {
      return fecha; // Retornar la fecha original si no se puede parsear
    }
