// facturas-compra-automatizada.component.ts - ACTUALIZADO SIN IVA PARA BOLETAS MANUALES
...
// Interface para factura manual ACTUALIZADA SIN IVA
interface FacturaManual {
  ...
  monto_total: number;
  monto_neto: number;
  // ELIMINADO: monto_exento y monto_iva_recuperable (sin IVA para boletas manuales)
  descripcion?: string;
...
@Component({
  selector: 'app-facturas-compra-automatizada',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './facturas-compra-automatizada.component.html',
  styleUrl: './facturas-compra-automatizada.component.css'
})
export class FacturasCompraAutomatizadaComponent implements OnInit, OnDestroy {
  
  // ==================== FORMULARIOS ====================
  
  distribucionForm: FormGroup;
  periodoForm: FormGroup;
  facturaManualForm: FormGroup; // ACTUALIZADO SIN CAMPOS DE IVA
  ...
  constructor(
    ...
    // FORMULARIO PARA FACTURA MANUAL ACTUALIZADO SIN IVA
    this.facturaManualForm = this.fb.group({
      rut_proveedor: ['', [Validators.required, Validators.pattern(/^\d{1,8}-[\dkK]$/)]],
      razon_social_proveedor: ['', [Validators.required, Validators.minLength(3)]],
      ...
      // ELIMINADO: monto_exento y monto_iva_recuperable
  ...
  async cargarProximoFolio(): Promise<void> {
    try {
      const response = await this.apiService.post('facturas_compra_automatico/', {
  ...
  // ==================== FACTURAS MANUALES ACTUALIZADAS SIN IVA ====================
  ...
  async confirmarCrearFacturaManual(): Promise<void> {
    ...
    try {
      ...
      const facturaManual: FacturaManual = {
        ...
        rut_proveedor: formData.rut_proveedor,
      ...
      if (response?.status === 'success') {
        this.mostrarExito('Boleta de compra creada exitosamente');
        this.cerrarDialogoFacturaManual();
        await this.cargarFacturas();
        await this.cargarProximoFolio();
      } else {
        this.mostrarError(response?.message || 'Error al crear la boleta de compra');
      }
    } catch (error: any) {
      console.error('Error al crear factura manual:', error);
      this.mostrarError('Error al crear boleta de compra: ' + (error.error?.message || error.message));
    } finally {
  ...
  // ACTUALIZADO: Cálculo simplificado sin IVA (igual que en ventas)
  ...
  esFacturaManual(tipoDoc: string): boolean {
    return ['BOLETA_MANUAL', 'FACTURA_MANUAL', 'NOTA_COMPRA', 'COMPROBANTE'].includes(tipoDoc);
  }
      ...
      if (response?.status === 'success') {
        ...
        this.facturas = (response.facturas || []).map((facturaRaw: any) => {
          const factura: FacturaDistribucion = {
            ...
            rut_proveedor: facturaRaw.rut_proveedor || '',
            razon_social: facturaRaw.razon_social || '',
            folio: facturaRaw.folio || '',
            fecha_docto: facturaRaw.fecha_docto || '',
            ...
            monto_iva_recuperable: Number(facturaRaw.monto_iva_recuperable) || 0,
  ...
  async cargarDatosDistribucion(): Promise<void> {
    try {
      const response = await this.apiService.obtenerDatosDistribucion().toPromise();
      if (response?.status === 'success') {
  ...
  async actualizarPeriodo(): Promise<void> {
    ...
    try {
      const configActual = await this.apiService.post('facturas_compra_automatico/', {
        action: 'get_automatic_configuration'
      }).toPromise();
  ...
  aplicarFiltros(): void {
    ...
    if (this.filtroTexto.trim()) {
      ...
      facturasFiltradas = facturasFiltradas.filter(f =>
        ...
        f.rut_proveedor.includes(texto) ||
  ...
  private async confirmarDistribucionSimple(): Promise<void> {
    ...
    const response = await this.apiService.post('facturas_compra_automatico/', formData).toPromise();
  ...
  private async confirmarDistribucionMultiple(): Promise<void> {
    ...
    for (const factura of this.facturasSeleccionadas) {
      try {
        ...
        const response = await this.apiService.post('facturas_compra_automatico/', dataFactura).toPromise();
  ...
  async onClienteSeleccionado(): Promise<void> {
    ...
    if (clienteId) {
      ...
      try {
        const response = await this.apiService.get(`api_campos_clientes/${clienteId}/`).toPromise();
  ...
  async visualizarPdfFactura(factura: FacturaDistribucion): Promise<void> {
    ...
    try {
      ...
      const pdfBlob = await this.apiService.getPDF(`facturas_sii_pdf/${factura.id}/`).toPromise();
  ...
  debugFactura(factura: FacturaDistribucion): void {
    console.group(`🔍 DEBUG FACTURA ID: ${factura.id}`);
    console.log('📄 Datos completos de la factura:', factura);
    console.log('📋 Información básica:');
    console.table({
      'ID': factura.id,
      'Folio': factura.folio,
      'Tipo Doc': factura.tipo_doc,
      'RUT Proveedor': factura.rut_proveedor,
      'Razón Social': factura.razon_social,
      'Fecha Documento': factura.fecha_docto,
      'Es Manual': factura.es_manual ? 'Sí' : 'No'
    });
    console.log('💰 Información de montos:');
    console.table({
      'Monto Total': factura.monto_total,
      'Monto Neto': factura.monto_neto,
      'Monto Exento': factura.monto_exento,
      'IVA Recuperable': factura.monto_iva_recuperable,
      'Monto Distribuido': factura.monto_distribuido,
      'Monto Pendiente': factura.monto_pendiente,
      'Porcentaje Distribuido': factura.porcentaje_distribuido + '%',
      'Porcentaje Pendiente': factura.porcentaje_pendiente + '%'
    });
    console.log('📊 Información de distribución:');
    console.table({
      'Completamente Distribuida': factura.completamente_distribuida,
      'Cantidad Distribuciones': factura.distribuciones_count,
      'Procesada': factura.procesada
    });
    console.log('📁 Información de PDF:');
    console.table({
      'PDF Disponible': factura.pdf_disponible,
      'PDF Descargado': factura.pdf_descargado,
      'URL PDF': factura.pdf_url,
      'Error Descarga': factura.error_descarga_pdf || 'Ninguno',
      'Intentos Descarga': factura.intentos_descarga_pdf,
      'Fecha Descarga': factura.fecha_descarga_pdf || 'No descargado'
    });
    if (factura.distribuciones && factura.distribuciones.length > 0) {
      console.log('🔄 Distribuciones existentes:');
      console.table(factura.distribuciones.map(dist => ({
        'ID': dist.id,
        'Cliente': dist.cliente.nombre,
        'Monto': dist.monto_distribuido,
        'Porcentaje': dist.porcentaje_distribuido + '%',
        'Fecha': dist.fecha_distribucion
      })));
    }
    console.groupEnd();
    
    this.snackBar.open(
      `Debug Factura ${factura.folio}: Ver consola para detalles completos`, 
      'Cerrar', 
  ...
  mostrarTodosLosDatos(): void {
    console.group('🗂️ DEBUG COMPLETO - TODAS LAS FACTURAS');
    console.log(`📊 Total de facturas cargadas: ${this.facturas.length}`);
    console.log(`📊 Total de facturas filtradas: ${this.facturasFiltradas.length}`);
    console.log(`📊 Facturas seleccionadas: ${this.facturasSeleccionadas.length}`);
    
    console.log('📈 Estadísticas PDF:', this.estadisticasPdf);
    console.log('📈 Estadísticas Distribución:', this.estadisticasDistribucion);
    
    if (this.modoDistribucionMultiple && this.distribucionMultiple) {
      console.log('🔄 Distribución múltiple:', this.distribucionMultiple);
    }
    
    console.log('📋 Todas las facturas:');
    this.facturas.forEach((factura, index) => {
      console.log(`--- Factura ${index + 1} ---`);
      console.log('ID:', factura.id);
      console.log('Folio:', factura.folio);
      console.log('Razón Social:', factura.razon_social);
      console.log('Monto Total:', factura.monto_total);
      console.log('PDF Disponible:', factura.pdf_disponible);
      console.log('Distribuciones:', factura.distribuciones_count);
      console.log('Seleccionada:', factura.selected);
      console.log('Es Manual:', factura.es_manual);
      console.log('---');
    });
    ...
    this.snackBar.open(
      'Datos mostrados en consola. Abre DevTools para verlos.', 
      'Cerrar', 
      { duration: 5000, panelClass: ['info-snackbar'] }
    );
  }
