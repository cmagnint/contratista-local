// parametros-facturas-compra.component.ts
...
@Component({
  selector: 'app-parametros-facturas-compra',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './parametros-facturas-compra.component.html',
  styleUrl: './parametros-facturas-compra.component.css'
})
export class ParametrosFacturasCompraComponent implements OnInit {
  ...
  try {
    const response = await this.contratistaApi.post('facturas_compra_automatico/', payload).toPromise();
  ...
  async probarConexion(): Promise<void> {
    ...
    try {
      ...
      if (response?.status === 'success') {
        ...
        const testResponse = await this.contratistaApi.post('facturas_compra_automatico/', testPayload).toPromise();
