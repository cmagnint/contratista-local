...
class AreaAdministracionAPIView(BaseAPIView):
    ...
    def get(self, request, format=None):
        ...
        if not holding_id:
            return Response(status=status.HTTP_400_BAD_REQUEST)
        areas = AreasAdministracion.objects.filter(holding_id=holding_id)
        return Response(AreaAdministracionSerializer(areas, many=True).data)
    ...
    def post(self, request, format=None):
        ...
        logger.error(f'AreaAdministracionAPIView POST: datos inválidos: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def delete(self, request, format=None):
        ids = request.data.get('ids', [])
        AreasAdministracion.objects.filter(id__in=ids).delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def patch(self, request, format=None):
        ...
        try:
            area = AreasAdministracion.objects.get(id=obj_id)
        except AreasAdministracion.DoesNotExist:
            logger.error(f'AreaAdministracionAPIView PATCH: area {obj_id} no encontrada')
            return Response({'message': 'Area no encontrado'}, status=status.HTTP_404_NOT_FOUND)

        serializer = AreaAdministracionSerializer(area, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)

        logger.error(f'AreaAdministracionAPIView PATCH: datos inválidos para area {obj_id}: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, format=None):
        ...
        try:
            ...
            logger.error(f'AreaAdministracionAPIView PUT: area {obj_id} no encontrada')
            return Response({'message': 'Perfil no encontrado'}, status=status.HTTP_404_NOT_FOUND)

        serializer = AreaAdministracionSerializer(area, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)

        logger.error(f'AreaAdministracionAPIView PUT: datos inválidos para area {obj_id}: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class CargoAdministracionAPIView(BaseAPIView):
    ...
    def get(self, request, format=None):
        ...
        return Response(CargoAdministracionSerializer(cargos, many=True).data)
    ...
    def post(self, request, format=None):
        ...
        logger.error(f'CargoAdministracionAPIView POST: datos inválidos: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def delete(self, request, format=None):
        ids = request.data.get('ids', [])
        CargosAdministracion.objects.filter(id__in=ids).delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def patch(self, request, format=None):
        ...
        try:
            cargo = CargosAdministracion.objects.get(id=obj_id)
        except CargosAdministracion.DoesNotExist:
            logger.error(f'CargoAdministracionAPIView PATCH: cargo {obj_id} no encontrado')
            return Response({'message': 'Perfil no encontrado'}, status=status.HTTP_404_NOT_FOUND)

        serializer = CargoAdministracionSerializer(cargo, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)

        logger.error(f'CargoAdministracionAPIView PATCH: datos inválidos para cargo {obj_id}: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def put(self, request, format=None):
        ...
        try:
            ...
            logger.error(f'CargoAdministracionAPIView PUT: cargo {obj_id} no encontrado')
            return Response({'message': 'Cargo no encontrado'}, status=status.HTTP_404_NOT_FOUND)

        serializer = CargoAdministracionSerializer(cargo, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)

        logger.error(f'CargoAdministracionAPIView PUT: datos inválidos para cargo {obj_id}: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
