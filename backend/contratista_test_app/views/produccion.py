...
class FiltrosProduccionTransferenciaAPIView(BaseAPIView):
    ...
    def get(self, request):
        try:
            ...
            registros_pagados = RegistroPagoTransferencia.objects.filter(
        ...
        try:
            ...
            with transaction.atomic():
                ...
                for trabajador_id, datos in pagos_por_trabajador.items():
                    ya_pagados = RegistroPagoTransferencia.objects.filter(
                        producciones__in=datos['registros']
                    ).exists()
                    if ya_pagados:
                        continue
                    ...
                    registro_pago.producciones.add(*datos['registros'])
...
# ==============================================================================
# GENERAR PLANILLA TRANSFERENCIA
# ==============================================================================

class GenerarPlanillaTransferenciaAPIView(BaseAPIView):
    ...
    def get(self, request):
        try:
            ...
            logger.debug(
                f'GenerarPlanillaTransferenciaAPIView GET: holding={holding_id} '
            ...
            logger.debug(f'GenerarPlanillaTransferenciaAPIView GET: {queryset.count()} registros')
            ...
            ancho_pagina = (landscape(A4)[0] - 10*mm) * 0.95
            DIAS_POR_FILA = 7
            ancho_dia = (ancho_pagina * 0.62) / DIAS_POR_FILA
            anchos_columnas = (
                [ancho_pagina * 0.20, ancho_pagina * 0.10]
            ...
            def prepare_table_data(trabajadores_data, dias):
                ...
                table_data = [['NOMBRE', 'RUT/NIC'] + dias_semana + ['TOTAL']]
                ...
                for worker_key, worker_data in trabajadores_data.items():
                    ...
                    for grupo in range(num_grupos):
                        ...
                        row.extend([f"{total:,.0f}"] if grupo == 0 else [''])
                    ...
                    table_data.append([''] * (DIAS_POR_FILA + 3))
                    current_row += 1
            ...
            header = Paragraph(
                f'<b>PLANILLA DE TRANSFERENCIAS</b><br/>'
            ...
            response['Content-Disposition'] = (
                f'attachment; filename=planilla_transferencia_{fecha_inicio.strftime("%Y%m%d")}.pdf'
            )
            return response

        except Exception as e:
            logger.error(f'GenerarPlanillaTransferenciaAPIView GET: error: {e}', exc_info=True)
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
