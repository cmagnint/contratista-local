...
class ContratoRetroactivoAPIView(BaseAPIView):
    ...
    def get(self, request):
        ...
        if not holding_id or not fecha_str:
            return Response({'error': 'holding y fecha requeridos'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            fecha = datetime.strptime(fecha_str, '%Y-%m-%d').date()
        except ValueError:
            return Response({'error': 'fecha debe ser YYYY-MM-DD'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for t in todos:
            tiene_contrato = ContratoTrabajador.objects.filter(
                trabajador_id=t.id, fecha_inicio_contrato__lte=fecha
            ).filter(
    ...
    def post(self, request):
        ...
        campos_requeridos = ['holding', 'trabajador', 'fecha_inicio_contrato', 'labor', 'folio_comercial', 'horario', 'fundo']
        faltantes = [c for c in campos_requeridos if not request.data.get(c)]
        if faltantes:
            return Response({'error': f'Faltan campos: {faltantes}'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            fecha_inicio = datetime.strptime(fecha_inicio_str, '%Y-%m-%d').date()
        except ValueError:
            return Response({'error': 'fecha_inicio_contrato debe ser YYYY-MM-DD'}, status=status.HTTP_400_BAD_REQUEST)

        tiene_contrato = ContratoTrabajador.objects.filter(
            trabajador_id=trabajador_id, fecha_inicio_contrato__lte=fecha_inicio
        ).filter(
            Q(fecha_termino_contrato__gte=fecha_inicio) | Q(fecha_termino_contrato__isnull=True)
        ).exists()

        if tiene_contrato:
            return Response({'error': 'El trabajador ya tiene un contrato vigente en esa fecha'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            folio = FolioComercial.objects.get(id=request.data['folio_comercial'], holding_id=holding_id)
        except FolioComercial.DoesNotExist:
            logger.error(f'ContratoRetroactivoAPIView POST: folio {request.data["folio_comercial"]} no encontrado')
        ...
        if request.data.get('fecha_termino_contrato'):
            try:
                fecha_termino = datetime.strptime(request.data['fecha_termino_contrato'], '%Y-%m-%d').date()
                contrato_data['fecha_termino_contrato'] = fecha_termino
            except ValueError:
                return Response({'error': 'fecha_termino_contrato debe ser YYYY-MM-DD'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if supervisor_id:
            try:
                ...
                historial_existente = SupervisorTrabajadorHistorial.objects.filter(
                    supervisor=supervisor, trabajador_id=trabajador_id, fecha_inicio__lte=fecha_inicio
                ).filter(
                ...
                if not historial_existente:
                    SupervisorTrabajadorHistorial.objects.create(
                        holding_id=holding_id, supervisor=supervisor,
                        trabajador_id=trabajador_id, fecha_inicio=fecha_inicio, fecha_fin=fecha_termino
                    )
            except Supervisores.DoesNotExist:
                logger.error(f'ContratoRetroactivoAPIView POST: supervisor {supervisor_id} no encontrado')
                return Response({'error': 'Supervisor no encontrado'}, status=status.HTTP_404_NOT_FOUND)
