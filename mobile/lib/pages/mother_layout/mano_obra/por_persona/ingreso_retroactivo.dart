...
import 'package:contratista/pages/mother_layout/mano_obra/por_persona/ingreso_mano_obra.dart'
    show Trabajador, UnidadControl;

class IngresoRetroactivoScreen extends StatefulWidget {
  ...
  const IngresoRetroactivoScreen({
    super.key,
  ...
  @override
  IngresoRetroactivoScreenState createState() =>
      IngresoRetroactivoScreenState();
}
  ...
  Future<void> enviarRegistros() async {
    ...
    for (var i = 0; i < widget.trabajadores.length; i++) {
      if (selectedTrabajadores[i] ?? false) {
        ...
        try {
          await apiService.post('gestion_retroactiva_mano_obra_persona/', data);
          loggerGlobal.d(
            'Registro retroactivo enviado: ${widget.trabajadores[i].id}',
          );
        } catch (e) {
          ...
          children: [
            ...
            Padding(
              ...
              child: ElevatedButton(
                ...
                child: const Text(
                  "GUARDAR REGISTROS RETROACTIVOS",
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
                  ),
